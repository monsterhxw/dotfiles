...
**方法一：**
>Chrome 完全直连访问，Chrome -直连-> `linux.do` 服务器
...
**方法二：**
>Chrome 「不」完全直连访问，Chrome -> Shadowrocket TUN -直连-> `linux.do` 服务器

- **绕过第一道「系统代理」拦截，配置 `skip-proxy (跳过代理)`**
- **「不」绕过第二道「TUN 捕获」的拦截，但配置直连规则 `DOMAIN-SUFFIX,cloudflare-ech.com,DIRECT`**
